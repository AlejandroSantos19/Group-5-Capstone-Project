...
if(!nameRegex.test(FirstName)){
  return "Invalid first name. Name must start with a capital letter and be less than 40 characters long";
}
...
//This function reads all employees from MySQL table and displays them in a table
...
try{
  ...
  document.getElementById("AverageHourlyWage").textContent = (stats.TotalHourlyWage / stats.TotalNumberEmployees).toFixed(2);
...
let myChart;
let EmployeePositionCountChart;
async function chart(){
Chart.defaults.font.family = 'Iosevka Charon Mono';
Chart.defaults.font.size = 18;
Chart.defaults.color = 'black';

 //This retrieves total number of employees, the total form the hourly wage column, the lowest hourly wage, and the highest hourly wage from our MySQl database. 
 //The results are saves into the stats variable
  const responseOne = await fetch("http://localhost:3000/stats");
  const stats = await responseOne.json();

  const responseTwo = await fetch("http://localhost:3000/employees");
  const employees = await responseTwo.json(); 

  const AverageHourlyWage = (stats.TotalHourlyWage / stats.TotalNumberEmployees).toFixed(2);
console.log("Chart is running");
//This checks if the charts already exists. If they do, they are destoryed and recreated again.
if(myChart && EmployeePositionCountChart){
  myChart.destroy();
  EmployeePositionCountChart.destroy();
}

const AverageHourlyWageChart = document.getElementById("AverageHourlyWageChart");
//These are counters used to keep track how many employees are earning above and below the average hourly wage.
let above = 0;
let below = 0;

//This loops through each employee and check if they are earning more or less than the average hourly wage
employees.forEach(employee => {
  if(employee.hourly_wage > Number(AverageHourlyWage))
  {
    //An employee is earning more than the average hourly wage
    above = above + 1;
  }
  else if(employee.hourly_wage < Number(AverageHourlyWage)){
    //An employee is earning less than the average hourly wage.
    below = below + 1;
  }
})

//This creates our pie chart in the Visual Graphs page.
myChart = new Chart(AverageHourlyWageChart, {
  type: "pie",
  data:{
    labels: ["Above", "Below"],
    datasets: [{
      data: [above, below],
      backgroundColor: ["#f7b888", "#79e89c"]
    }]
  }
});

//This is an array used to keep track how many employees are there for each position
const positionCount = {};
//This loops through all the employee and counts how many employees are there of each position.
employees.forEach(employee => {
...
const positionData = Object.values(positionCount);

const EmployeePositionsCount = document.getElementById("EmployeePositionsCount");

//This creates the bar graph in the visual graphs page.
EmployeePositionCountChart = new Chart(EmployeePositionsCount, {
  type: "bar",
data: {
